...
def create_meeting(data: MeetingCreate, request: Request, user=Depends(current_user)):
    if not data.consent_confirmed:
        raise HTTPException(status_code=400, detail="Recording consent confirmation required")
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            plan = normalize_plan((cur.fetchone() or {}).get("plan", "free"))
            if plan == "free":
                cur.execute("""
                ...
                """, (user["id"],))
                used = int((cur.fetchone() or {}).get("count", 0))
                if used >= 3:
                    raise HTTPException(status_code=402, detail="Free plan limit reached: 3 meetings/month. Upgrade to Premium or Team.")
            cur.execute(
                """
                INSERT INTO meetings(user_id,title,meeting_link,platform,consent_confirmed,status)
                VALUES(%s,%s,%s,%s,%s,%s)
                RETURNING id
                """,
                (user["id"], data.title, data.meeting_link, data.platform, data.consent_confirmed, "created")
            )
            meeting_id = cur.fetchone()["id"]
            cur.execute(
                """
                INSERT INTO consent_logs(meeting_id,user_id,consent_text,ip_address,user_agent)
                VALUES(%s,%s,%s,%s,%s)
                """,
                (meeting_id, user["id"], consent_text, request.client.host if request.client else None, request.headers.get("user-agent"))
            )
    return {"id": meeting_id, "message": "Meeting created with recording consent log", "consent_notice": consent_text}

@router.get("")
def list_meetings(user=Depends(current_user)):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM meetings WHERE user_id=%s ORDER BY created_at DESC", (user["id"],))
            rows = cur.fetchall()
...
def start_bot(meeting_id: int, user=Depends(current_user)):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM meetings WHERE id=%s AND user_id=%s", (meeting_id, user["id"]))
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO bot_jobs(meeting_id,job_id,status,message,started_at) VALUES(%s,%s,%s,%s,NOW())",
                (meeting_id, job_id, status, message)
            )
            cur.execute("UPDATE meetings SET status=%s WHERE id=%s", ("bot_started", meeting_id))
    return {"job_id": job_id, "status": status, "message": message}

@router.post("/{meeting_id}/recording")
def upload_recording(meeting_id: int, file: UploadFile = File(...), user=Depends(current_user)):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM meetings WHERE id=%s AND user_id=%s", (meeting_id, user["id"]))
            meeting = cur.fetchone()
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (meeting_id, user["id"], file.filename or safe_name, str(path), file.content_type, storage.get("storage_provider","local"), storage.get("storage_url"), True, True, True)
            )
            cur.execute(
                """
                ...
                """,
                (
                    meeting_id, transcript, report.get("summary", ""), report.get("key_points", ""),
                    report.get("decisions", ""), report.get("action_items", ""), report.get("deadlines", ""),
                    report.get("language", "auto"), int(report.get("productivity_score", 0) or 0), int(report.get("waste_score", 0) or 0), json.dumps(report.get("speakers", [])), json.dumps(report.get("analytics", {})), json.dumps(pipeline.get("translation", report.get("translation", {})))
                )
            )
            cur.execute("UPDATE meetings SET status=%s WHERE id=%s", ("processed", meeting_id))
    return {"message": "Recording uploaded and processed", "report": report}
...
def get_report(meeting_id: int, user=Depends(current_user)):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            if not meeting:
                raise HTTPException(status_code=404, detail="Meeting not found")
            cur.execute("SELECT * FROM ai_reports WHERE meeting_id=%s", (meeting_id,))
            report = cur.fetchone()
    return {"meeting": row_to_dict(meeting), "report": row_to_dict(report)}
